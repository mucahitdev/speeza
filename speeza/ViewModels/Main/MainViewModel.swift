...
class MainViewModel: ObservableObject {
    ...
    func loadNote(by id: UUID, from notes: [TextNote]) {
        if let note = notes.first(where: { $0.id == id }) {
            self.text = note.text
            self.title = note.title
            self.selectedLanguage = note.language
            self.selectedVoice = note.voice
            self.rate = note.rate
            self.selectedGroupID = note.groupID
        }
    }
    
    func updateNote(id: UUID, modelContext: ModelContext, notes: [TextNote]) {
        if let note = notes.first(where: { $0.id == id }) {
            note.text = self.text
            note.title = self.title
            note.language = self.selectedLanguage
            note.voice = self.selectedVoice
            note.rate = self.rate
            note.groupID = self.selectedGroupID
            note.updatedAt = Date()
    ...
    func hasChanges(comparedTo note: TextNote) -> Bool {
    ...
    func revertChanges(from note: TextNote) {
        self.text = note.text
        self.title = note.title
        self.selectedLanguage = note.language
        self.selectedVoice = note.voice
